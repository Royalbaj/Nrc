function saveContact() {
    ...
    const org = profileCard.querySelector('.profile-institution')?.innerText || '';
    const title = profileCard.querySelector('.profile-post')?.innerText || '';
    const workPhone = profileCard.querySelector('.profile-contact-info a[href^="tel:"]')?.innerText.split(': ')[1] || '';
    const cellPhone = profileCard.getAttribute('data-phone') || '';
    const email = profileCard.getAttribute('data-email') || '';
    const address = profileCard.querySelector('.profile-location')?.innerText.split(': ')[1] || '';
...
END:VCARD`;
    ...
    a.href = blobURL;
    a.setAttribute('download', `${name.replace(/ /g, '_')}_contact.vcf`);
